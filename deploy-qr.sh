...
echo "=== Deploying QR-Code Service & Admin (T-037/T-038) ==="

# 1. Install qrcode package
echo "1/5 Installing qrcode..."
npm install qrcode@1.5.4 --save
npm install @types/qrcode --save-dev

# 2. Create API routes
echo "2/5 Creating API routes..."
mkdir -p src/app/api/v1/qr-codes

# === QR Code API: List + Create ===
cat > src/app/api/v1/qr-codes/route.ts << 'ENDFILE'
...
# === QR Code API: Generate image ===
mkdir -p src/app/api/v1/qr-codes/generate
cat > src/app/api/v1/qr-codes/generate/route.ts << 'ENDFILE'
import { NextRequest, NextResponse } from 'next/server';
...
# === QR Code API: Delete + Toggle ===
mkdir -p "src/app/api/v1/qr-codes/[id]"
cat > "src/app/api/v1/qr-codes/[id]/route.ts" << 'ENDFILE'
import { NextRequest, NextResponse } from 'next/server';
import { getServerSession } from 'next-auth';
import { authOptions } from '@/lib/auth';
import prisma from '@/lib/prisma';
...
# 3. Create Admin QR page
echo "3/5 Creating admin page..."

cat > src/app/admin/qr-codes/page.tsx << 'ENDFILE'
import { getServerSession } from 'next-auth';
import { authOptions } from '@/lib/auth';
import prisma from '@/lib/prisma';
...
# 4. Create QR Admin Client Component
echo "4/5 Creating client component..."
mkdir -p src/components/admin

cat > src/components/admin/qr-code-admin.tsx << 'ENDFILE'
'use client';
...
echo ""
echo "=== QR-Code Service deployed! ==="
echo ""
echo "Features:"
echo "  - Admin: /admin/qr-codes (Login erforderlich)"
echo "  - Liste aller QR-Codes mit Preview, Scan-Counter"
echo "  - Erstellen: Label, Standort, Karte, Farben"
echo "  - Download PNG/SVG (1024px)"
echo "  - Aktivieren/Deaktivieren"
echo "  - Redirect: /q/{code} → Karte"
echo "  - API: /api/v1/qr-codes/generate?url=...&format=png|svg"
