...
export default async function DashboardPage() {
  ...
  const [menuCount, productCount, qrCount, mediaCount, groupCount, translationCount] = await Promise.all([
    ...
    prisma.qRCode.count({ where: { menu: { location: { tenantId: tid } } } }),
    prisma.media.count({ where: { tenantId: tid } }),
    prisma.taxonomyNode.count({ where: { tenantId: tid, type: 'CATEGORY' } }),
          ...
          >
            ...
            <div className="space-y-3">
              {recentProducts.map((product) => {
                ...
                const defVariant = (product as any).variants?.[0];
                const price = defVariant?.prices?.[0]?.sellPrice ? `${Number(defVariant.prices[0].sellPrice).toFixed(2).replace('.', ',')} €` : '';
                const thumb = (product.productMedia[0] as any)?.url;
