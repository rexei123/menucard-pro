...
echo "================================================"
echo "SCHRITT 4b: Werkstatt neu (Tabs + Aktionen)"
echo "================================================"

cp src/app/admin/design/page.tsx src/app/admin/design/page.tsx.bak-4b

cat > src/app/admin/design/page.tsx <<'TSXEOF'
'use client';
...
/* ──────────────────────────────────────────────────────────────
   Template-Meta nach baseType (fuer Untertitel/Beschreibung/Tags)
   Werte 1:1 aus src/lib/design-templates/*.ts
   ────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   Mini-Vorschau (1:1 zu den Template-Configs)
   ────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   Aktions-Button (kompakt, icon + label)
   ────────────────────────────────────────────────────────────── */
function ActionBtn({
  icon, label, color, onClick, disabled, title,
}: { icon: string; label: string; color?: string; onClick: () => void; disabled?: boolean; title?: string }) {
  return (
    <button
    ...
    >
      ...
      <div className="p-5" style={{ borderTop: '1px solid #F3F3F6' }}>
        <div className="flex items-start justify-between mb-1">
          <h3 className="text-lg font-bold" style={{ fontFamily: "'Playfair Display', serif", color: '#171A1F' }}>
            {tpl.name}
          </h3>
        </div>
...
function DuplicateDialog({
  source, onClose, onConfirm,
}: { source: DesignTemplate; onClose: () => void; onConfirm: (name: string) => Promise<void> }) {
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                setError(e.message || 'Fehler beim Duplizieren');
              } finally {
                setBusy(false);
              }
            }}
...
/* ──────────────────────────────────────────────────────────────
   Bestaetigungs-Dialog (Archiv/Wiederherstellen)
   ────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   Vergleichstabelle (1:1 uebernommen)
   ────────────────────────────────────────────────────────────── */
...
export default function DesignOverviewPage() {
  ...
  /* Gruppierung nach templateId (eindeutig pro DB-Template) */
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: '#999' }}>
                  Öffnen Sie eine Karte, um ein Template auszuwählen.
                </div>
              </div>
            </div>
            <button
...
echo "  OK: Werkstatt neu geschrieben"
...
echo ""
echo "--- Test: /api/v1/design-templates ---"
curl -s "http://localhost:3000/api/v1/design-templates" -o /dev/null -w "HTTP %{http_code}\n"

echo ""
echo "================================================"
echo "SCHRITT 4b FERTIG"
echo "Bitte /admin/design mit Strg+Shift+R neu laden."
echo "================================================"
