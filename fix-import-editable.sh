#!/bin/bash
# MenuCard Pro – CSV-Import: Editierbare Vorschau
# Datum: 10.04.2026

set -e
cd /var/www/menucard-pro

echo "=== CSV-Import: Editierbare Vorschau ==="

cat > src/components/admin/csv-import.tsx << 'ENDOFFILE'
'use client';
...
type Edits = Record<string, Edit[]>; // sku -> edits
...
// Map UI field names to CSV column headers
...
function applyCsvEdits(rawText: string, edits: Edits): string {
  ...
  // For each SKU with edits, find and modify matching rows
  for (const [sku, editList] of Object.entries(edits)) {
    // Find the CSV column index for sku
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (cols[skuIdx]?.trim() !== sku) continue;

      // This row matches the SKU - apply edits
      for (const edit of editList) {
        const possibleHeaders = fieldToCsvCol[edit.field] || [edit.field];
        const colIdx = headers.findIndex(h => possibleHeaders.includes(h));
        if (colIdx !== -1 && colIdx < cols.length) {
          cols[colIdx] = edit.value;
        }
      }
...
export default function CsvImport() {
  ...
  const handleCellEdit = (sku: string, field: string, value: string) => {
    setEdits(prev => {
      const skuEdits = [...(prev[sku] || [])];
      const existing = skuEdits.findIndex(e => e.field === field);
      if (existing >= 0) {
        skuEdits[existing] = { field, value };
      } else {
        skuEdits.push({ field, value });
      }
      return { ...prev, [sku]: skuEdits };
    });
    setHasEdits(true);
  };

  const getEditedValue = (sku: string, field: string, original: string): string => {
    const skuEdits = edits[sku];
    if (!skuEdits) return original;
    const edit = skuEdits.find(e => e.field === field);
    return edit ? edit.value : original;
  };
  ...
  const EditableCell = ({ sku, field, value, isError, className = '' }: {
    sku: string; field: string; value: string; isError: boolean; className?: string;
  }) => {
    const edited = getEditedValue(sku, field, value);
    const wasEdited = edited !== value;
    ...
    if (field === 'type') {
      return (
        <select
          value={edited}
          onChange={e => handleCellEdit(sku, field, e.target.value)}
        ...
        >
          ...
          {!['WINE', 'DRINK', 'FOOD'].includes(value) && <option value={value} disabled>{value} (ungueltig)</option>}
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y">
                  {products.map((p, i) => {
                    ...
                    return (
                      <tr key={i} className={isError ? 'bg-red-50/50' : ''}>
                        ...
                        <td className="px-3 py-2">
                          <EditableCell sku={p.sku} field="type" value={p.type} isError={isError} />
                        </td>
                        <td className="px-3 py-2">
                          <EditableCell sku={p.sku} field="nameDe" value={p.nameDe} isError={isError} className="font-medium" />
                        </td>
                        <td className="px-3 py-2 tabular-nums">
                          <EditableCell sku={p.sku} field="price" value={p.price || '-'} isError={isError} />
                        </td>
                        <td className="px-3 py-2 text-xs text-gray-500 max-w-[200px]">
                          {isError ? (
                            <span className="text-red-600">{p.statusMsg}</span>
                          ) : (
                            <span className="truncate block">{p.statusMsg}</span>
                          )}
                        </td>
                      </tr>
                    );
...
echo "[1/2] Editierbare Vorschau-Komponente erstellt"
...
echo ""
echo "=== Fertig! ==="
echo "CSV-Import hat jetzt editierbare Fehlerzellen."
echo "Bei Fehlern: Wert korrigieren > 'Neu validieren' klicken > importieren."
