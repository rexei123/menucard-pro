...
export default function ProductEditor({ product: initial, options }: { product: ProductData; options: Options }) {
  ...
  return (
    <div className="space-y-6 max-w-4xl pb-24">
      {/* Header */}
      <div>
        <a href="#" onClick={(e) => { e.preventDefault(); if (!dirty || confirm("Ungespeicherte Änderungen verwerfen?")) window.location.href="/admin/items"; }} className="text-xs text-gray-400 hover:text-gray-600 mb-2 inline-block">← Alle Produkte</a>
      ...
      {(dirty || saved || error) && <div className="fixed bottom-0 left-0 right-0 border-t bg-white/95 backdrop-blur-md px-6 py-3 flex items-center justify-between z-50 animate-in slide-in-from-bottom">
        ...
        <div className="flex gap-2">
          <button onClick={() => { if (!dirty || confirm("Ungespeicherte Änderungen verwerfen?")) window.location.href="/admin/items"; }} className="rounded-lg border px-4 py-2 text-sm font-medium hover:bg-gray-50">Abbrechen</button>
