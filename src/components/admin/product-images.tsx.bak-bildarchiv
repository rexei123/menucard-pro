...
export default function ProductImages({ productId, initialImages }: { productId: string; initialImages: ImageData[] }) {
  ...
  const setPrimary = async (id: string) => {
    ...
    if (res.ok) setImages(prev => { const updated = prev.map(img => ({ ...img, isPrimary: img.id === id })); const primary = updated.find(i => i.id === id); if (primary) { const rest = updated.filter(i => i.id !== id); return [primary, ...rest]; } return updated; });
  };
