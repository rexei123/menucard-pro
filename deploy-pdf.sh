...
echo "=== Deploying PDF Export (T-040/T-041) ==="

# 1. Install react-pdf
echo "1/4 Installing @react-pdf/renderer..."
npm install @react-pdf/renderer@4.3.0 --save

# 2. Create PDF API route
echo "2/4 Creating PDF API..."
mkdir -p src/app/api/v1/pdf

cat > src/app/api/v1/pdf/route.tsx << 'ENDTSX'
...
# 3. Add PDF download button to admin menus page
echo "3/4 Checking admin menus page..."
# We'll add a simple PDF link to the QR admin since menus page structure is unknown
# For now the PDF is accessible via API URL directly
...
echo ""
echo "=== PDF Export deployed! ==="
echo ""
echo "Usage:"
echo "  /api/v1/pdf?menu=weinkarte                    (Weinkarte DE)"
echo "  /api/v1/pdf?menu=weinkarte&lang=en             (Wine List EN)"
echo "  /api/v1/pdf?menu=barkarte                      (Barkarte DE)"
echo "  /api/v1/pdf?menu=jaegerabend                   (Jägerabend DE)"
echo ""
echo "Test: http://178.104.138.177/api/v1/pdf?menu=weinkarte"
