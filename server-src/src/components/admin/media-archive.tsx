...
function UploadTab({ onUploaded }: { onUploaded: () => void }) {
  ...
  function statusIcon(status: UploadItem['status']) {
    if (status === 'done')      return <MIcon name="check_circle" size={20} className="text-green-600" />;
    if (status === 'error')     return <MIcon name="error"        size={20} className="text-red-600" />;
    if (status === 'uploading') return <MIcon name="hourglass_top" size={20} className="text-amber-600" />;
    return <MIcon name="description" size={20} className="text-[#565D6D]" />;
  }
    ...
    <div>
      ...
      >
        <div className="mb-3 flex justify-center">
          <MIcon name="add_a_photo" size={48} className="text-[#565D6D]" />
        </div>
        ...
        <div className="mt-4 space-y-2">
          {files.map((item, i) => (
            <div key={i} className="flex items-center gap-3 p-2 bg-[#F9FAFB] rounded-lg">
              <span className="flex items-center">{statusIcon(item.status)}</span>
...
export default function MediaArchive() {
  ...
  const tabs = [
    { id: 'photos' as const, icon: 'photo_camera', label: 'Fotos' },
    { id: 'logos'  as const, icon: 'sell',         label: 'Logos' },
    { id: 'upload' as const, icon: 'upload',       label: 'Hochladen' },
    { id: 'web'    as const, icon: 'language',     label: 'Websuche' },
  ];
